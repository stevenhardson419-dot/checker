...
try:
    ...
    USE_CURL_CFFI = True
    print("[TLS] curl_cffi loaded - using Chrome TLS fingerprint impersonation")
except ImportError:
    print("[TLS] curl_cffi not available - using standard requests (higher CAPTCHA risk)")
    curl_requests = None
...
CHROME_FINGERPRINTS = [
    ...
    },
    {
        ...
        "sec_ch_ua": '"Google Chrome";v="129", "Chromium";v="129", "Not=A?Brand";v="99"',
        "sec_ch_ua_platform": '"Windows"',
        "sec_ch_ua_mobile": "?0"
    },
    {
...
_fingerprint_local = threading.local()

def get_browser_fingerprint() -> dict:
    """Get or create a consistent browser fingerprint for this thread/session"""
    if not hasattr(_fingerprint_local, 'fingerprint') or _fingerprint_local.fingerprint is None:
        _fingerprint_local.fingerprint = random.choice(CHROME_FINGERPRINTS)
    return _fingerprint_local.fingerprint

def refresh_browser_fingerprint() -> dict:
    """Get a new random browser fingerprint for this thread"""
    _fingerprint_local.fingerprint = random.choice(CHROME_FINGERPRINTS)
    return _fingerprint_local.fingerprint

def get_random_user_agent() -> str:
    """Get consistent user agent from current fingerprint"""
    return get_browser_fingerprint()["user_agent"]

def get_sec_ch_ua_headers() -> dict:
    """Get consistent Sec-CH-UA headers matching current fingerprint"""
...
def get_graphql_headers(checkout_token: str, session_token: str, shop_url: str = None) -> dict:
    ...
    return {
        ...
        'Origin': origin,
        'Referer': f'{origin}/checkouts/cn/{checkout_token}',
        'Sec-CH-UA': fp["sec_ch_ua"],
        'Sec-CH-UA-Mobile': fp["sec_ch_ua_mobile"],
        'Sec-CH-UA-Platform': fp["sec_ch_ua_platform"],
...
def load_user_agents():
    """Load user agents from file - now using CHROME_FINGERPRINTS instead"""
    global USER_AGENTS
    USER_AGENTS = [fp["user_agent"] for fp in CHROME_FINGERPRINTS]

ADDRESSES = []
ADDRESSES_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "addresses.txt")
HARDCODED_PHONE = "2494851515"  # Keep this phone number for all addresses

def load_addresses():
    """Load addresses from file. Format: FirstName|LastName|Email|Address|City|State|Zip|Country|Phone|Lat|Long"""
    global ADDRESSES
    try:
                    ...
                    if line and "|" in line:
                        ...
                        if len(parts) >= 11:
                            ADDRESSES.append({
                                ...
                                "zip": parts[6],
                                "country": parts[7],
                                "phone": HARDCODED_PHONE,  # Use hardcoded phone, not from file
                                "latitude": float(parts[9]) if parts[9] else 40.7589,
                                "longitude": float(parts[10]) if parts[10] else -73.9851
                            })
            if ADDRESSES:
                print(f"[ADDRESSES] Loaded {len(ADDRESSES)} addresses from file")
    except Exception as e:
        print(f"[ADDRESSES] Failed to load addresses: {e}")

def get_random_address() -> dict:
    """Get a random address from the pool, or default if pool is empty"""
    if not ADDRESSES:
        load_addresses()
...
def get_checkout_data() -> dict:
    """Get checkout data for the current thread. Creates a new random address if not set."""
    if not hasattr(_thread_local, 'checkout_data') or _thread_local.checkout_data is None:
        _thread_local.checkout_data = get_random_address()
    return _thread_local.checkout_data

def refresh_checkout_data():
    """Refresh checkout data with a new random address for the current thread"""
    _thread_local.checkout_data = get_random_address()
    return _thread_local.checkout_data

class _CheckoutDataProxy:
    """Proxy class that makes CHECKOUT_DATA thread-safe by using thread-local storage"""
    def __getitem__(self, key):
        return get_checkout_data()[key]
    
    def get(self, key, default=None):
        return get_checkout_data().get(key, default)
    
    def __contains__(self, key):
        return key in get_checkout_data()
    
    def keys(self):
        return get_checkout_data().keys()
    
    def values(self):
        return get_checkout_data().values()
    
    def items(self):
        return get_checkout_data().items()

CHECKOUT_DATA = _CheckoutDataProxy()
...
def create_session(shop_url, proxies=None):
    ...
    CHROME_IMPERSONATE_TARGETS = ["chrome131", "chrome124", "chrome123"]
...
def step1_add_to_cart(session):
    ...
    try:
        ...
        # Debug: check if add to cart actually worked
        try:
            cart_response = r.json()
            if 'items' in cart_response:
                item_count = len(cart_response['items'])
                print(f"  [DEBUG] Cart now has {item_count} items")
                if item_count == 0:
                    print(f"  [WARNING] Add to cart failed - cart is empty")
                    return None, None, None
            else:
                print(f"  [DEBUG] Cart response: {str(cart_response)[:200]}...")
        except Exception as e:
            print(f"  [DEBUG] Could not parse cart response: {e}")
...
def extract_session_token(html):
    ...
    # Try multiple patterns in case the format varies
    patterns = [
        r'<meta\s+name="serialized-sessionToken"\s+content="([^"]+)"',
        r'<meta\s+name="serialized-session-token"\s+content="([^"]+)"',
        r'<meta\s+name="[^"]*session[^"]*token[^"]*"\s+content="([^"]+)"',
        r'serialized-sessionToken["\s]*:\s*["\']([^"\']+)["\']',
        r'sessionToken["\s]*:\s*["\']([^"\']+)["\']'
    ]
    ...
    for pattern in patterns:
        ...
        if meta_match:
            ...
            token = content.strip('"\'')
            if len(token) > 50:
                print(f"  [OK] Session token extracted (pattern: {pattern[:30]}...)")
                return token
    
    # Debug: show a snippet of the HTML around potential session tokens
    if 'session' in html.lower() and 'token' in html.lower():
        # Find all potential session token related content
        session_matches = re.findall(r'session.*?token.*?(?:["\':]\s*([^"\s]+))?', html, re.IGNORECASE | re.DOTALL)
        if session_matches:
            print(f"  [DEBUG] Found potential session token patterns: {session_matches[:3]}")
    
    print("  [WARNING] Session token not found in HTML")
    return None
...
SHORT_SLEEP = 1.5  # Increased to reduce CAPTCHA detection
MAX_WAIT_SECONDS = 4.0  # Slightly increased for more human-like behavior
HTTP_TIMEOUT_SHORT = 15  # Increased timeout
HTTP_TIMEOUT_MEDIUM = 20  # Increased timeout
...
def step1_add_to_cart_ctx(session, shop_url, variant_id, _429_retry_count=0):
    refresh_checkout_data()
    
    refresh_browser_fingerprint()
    ...
    try:
        if _429_retry_count > 0:
            delay = 3.0 + (_429_retry_count * 2.0)  # Increased base delay
            jitter = random.uniform(0.5, 1.5)  # Increased jitter
            time.sleep(delay + jitter)
        else:
            time.sleep(random.uniform(0.5, 1.2))  # Increased from 0.2-0.5
        ...
        raise
    time.sleep(random.uniform(1.2, 2.0))  # Increased from 0.8-1.5
    
    checkout_url = f"{shop_url}/checkout"
    try:
        print(f"  [DEBUG] Requesting checkout URL: {checkout_url}")
        r = session.get(checkout_url, allow_redirects=True, timeout=HTTP_TIMEOUT_SHORT, verify=False)
        print(f"  [DEBUG] Checkout URL response status: {r.status_code}")
    except requests.exceptions.ProxyError as e:
        error_msg = str(e).lower()
        if "429" in error_msg or "too many requests" in error_msg:
            if _429_retry_count < 1:
    ...
    if '/checkouts/cn/' in final_url:
        ...
        if 'serialized-session' in r.text.lower():
            print(f"  [DEBUG] Found 'serialized-session' in HTML")
        else:
            print(f"  [DEBUG] 'serialized-session' NOT found in HTML, checking for alternatives...")
            # Show a snippet of the HTML for debugging
            html_preview = r.text[:500] + "..." if len(r.text) > 500 else r.text
            print(f"  [DEBUG] HTML preview: {html_preview}")
        
        session_token = extract_session_token(r.text)
        return checkout_token, session_token, r.cookies
    else:
        print(f"  [WARNING] Final URL doesn't contain '/checkouts/cn/': {final_url}")
        return None, None, None
