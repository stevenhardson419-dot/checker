...
# BIN info cache to avoid repeated API calls
...
def classify_prefix(code_display: str) -> str:
    ...
    if ('"ACTION_REQUIRED"' in u) or ('ACTION_REQUIRED' in u) or ('3D' in u):
        return "approved"
    # CAPTCHA_REQUIRED is treated as unknown (retryable), not declined
    # This allows it to be retried on different sites, and if all sites fail with CAPTCHA,
    # the batch runner will handle it specially
